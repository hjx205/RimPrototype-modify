...
namespace Hjx_BiomassNucleus
{
    ...
    public static class AbilityDefOf
    {
        ...
        public static AbilityDef BMN_Evolution;
    ...
    public class CompAbilityEffect_GiveAdaptive : CompAbilityEffect_WithDuration
    {
    ...
    public class CompAbilityEffect_Integrate : CompAbilityEffect
    {
        ...
        public override void Apply(LocalTargetInfo target, LocalTargetInfo dest)
        {
            ...
            if (positionHeld.IsValid)
            {
                ...
                if (parent.pawn.health?.hediffSet?.hediffs?.FirstOrDefault((Hediff h) => h is Hediff_BiomassNucleus) is Hediff_BiomassNucleus hediff_BiomassNucleus)
                {
                    ...
                    {
                        //增加同化后可以吸收经验的设定
                        float xpTransferPercent = (float)Math.Min(hediff_BiomassNucleus.Severity, 0.5);
                        float xpTransfer;
                        SkillDef IntegrateSkillAndXpTransfer = PsychicRitualUtility.GetPhilophagySkillAndXpTransfer(parent.pawn, pawn, xpTransferPercent, out xpTransfer);
                        if (IntegrateSkillAndXpTransfer != null)
                        {
                            int level = parent.pawn.skills.GetSkill(IntegrateSkillAndXpTransfer).GetLevel();
                            parent.pawn.skills.Learn(IntegrateSkillAndXpTransfer, xpTransfer, direct: true, ignoreLearnRate: true);
                            int level2 = parent.pawn.skills.GetSkill(IntegrateSkillAndXpTransfer).GetLevel();
                            TaggedString resultText = string.Format("{0}", "PawnGainedXPInSkill".Translate(parent.pawn, xpTransfer, IntegrateSkillAndXpTransfer)) +
                                (", " + ((level2 > level) ? "PhilophagySkillLevelGained".Translate(level, level2).Resolve() : "PhilophagySkillLevelRemains".Translate(level2).Resolve()));
                            Find.LetterStack.ReceiveLetter("IntegrateImmunityComplete".Translate(), resultText, LetterDefOf.PositiveEvent, new LookTargets(parent.pawn, pawn));
                        }
                    }
                }
            }
        ...
        public override string ExtraLabelMouseAttachment(LocalTargetInfo target)
        {
            ...
            if (pawn != null)
            {
                ...
                {
                    if (pawn.def.defName == "Human" && ModsConfig.BiotechActive)
                    {
                        if (hediff_BiomassNucleus.RaceDictionary.ContainsKey(pawn.genes.Xenotype.defName))
                        {
                            return null;
                        }
                    }
    ...
    public class CompUseEffect_BiomassNucleus : CompUseEffect
    {
        ...
        public override void DoEffect(Pawn user)
        {
            ...
            if (user.health.hediffSet.HasHediff(HediffDefOf.Hediff_ParenchymaCore))
            {
                Hediff remove_h = user.health.hediffSet.GetFirstHediffOfDef(HediffDefOf.Hediff_ParenchymaCore);
                user.health.hediffSet.hediffs.Remove(remove_h);
            }
            if (user.health?.hediffSet?.hediffs?.FirstOrDefault((Hediff h) => h is Hediff_BiomassNucleus) is Hediff_BiomassNucleus hediff_BiomassNucleus)
            {
                ...
                {
                    GeneDef black_light_r = DefDatabase<GeneDef>.GetNamed("BMN_Black_Light_Recessive");
                    GeneDef clotting = DefDatabase<GeneDef>.GetNamed("Superclotting");
                    user.genes.AddGene(black_light_r, false);
                    user.genes.AddGene(clotting, true);
                }
                copypawnID = null;
            }
        }
        ...
        public override GizmoResult GizmoOnGUI(Vector2 topLeft, float maxWidth, GizmoRenderParms parms)
        {
            ...
            if (Widgets.ButtonImage(butRect, shieldIcon, doMouseoverSound: true, "shieldIcon_desc".Translate()))
            {
                ...
                if (nucleus.shieldOn)
                {
                    ...
                    if (nucleus.pawn.SpawnedOrAnyParentSpawned)
                    {
                        if(nucleus.pawn.equipment.Primary != null && nucleus.pawn.equipment.TryDropEquipment(nucleus.pawn.equipment.Primary, out var dropped, nucleus.pawn.PositionHeld)){
                            dropped?.SetForbidden(false);
                        }
                    }
    ...
    public class Hediff_BiomassNucleus : HediffWithComps
    {
        ...
        private static bool Lv1_trait_gain = false;

        private static bool Lv2_trait_gain = false;

        private static bool Lv3_trait_gain = false;

        private static bool Lv4_trait_gain = false;

        private static bool Lv5_trait_gain = false;

        private static bool Lv6_trait_gain = false;

        private static bool Lv7_trait_gain = false;

        private static bool Lv8_trait_gain = false;
        ...
        public override void ExposeData()
        {
            ...
            Scribe_Values.Look(ref Lv1_trait_gain, "Lv1_trait_gain", defaultValue: false);
            Scribe_Values.Look(ref Lv2_trait_gain, "Lv2_trait_gain", defaultValue: false);
            Scribe_Values.Look(ref Lv3_trait_gain, "Lv3_trait_gain", defaultValue: false);
            Scribe_Values.Look(ref Lv4_trait_gain, "Lv4_trait_gain", defaultValue: false);
            Scribe_Values.Look(ref Lv5_trait_gain, "Lv5_trait_gain", defaultValue: false);
            Scribe_Values.Look(ref Lv6_trait_gain, "Lv6_trait_gain", defaultValue: false);
            Scribe_Values.Look(ref Lv7_trait_gain, "Lv7_trait_gain", defaultValue: false);
            Scribe_Values.Look(ref Lv8_trait_gain, "Lv8_trait_gain", defaultValue: false);
        ...
        public void UpdateTargetPawnDef(Pawn target)
        {
            if (target != null)
            {
                ...
                if (ModsConfig.BiotechActive && text == "Human")
                {
                    text = target?.genes?.Xenotype?.defName;
                    text2 = target?.genes?.XenotypeLabel;
                }
                if (text == null)
                {
                    text = "Unknow";
        ...
        {
            Severity = (float)RaceCount * 0.016f;
            energyMax = 200f + (float)RaceCount * 10f;
            UpdateClawL();
            UpdateClawR();
            UpdateBlade();
            //[测试]添加升级后获得特质的能力 Lv.1 超免疫者
            if (0.05f <= Severity && Severity < 0.1f && !Lv1_trait_gain)
            {
                var trait_lv1 = new Trait(DefDatabase<TraitDef>.GetNamed("Immunity"), 1);
                pawn.story.traits.GainTrait(trait_lv1);
                TaggedString resultText = string.Format("{0}", "BMN_GainTrait_text".Translate(pawn, trait_lv1.Label));
                Find.LetterStack.ReceiveLetter("BMN_GainTrait".Translate(), resultText, LetterDefOf.PositiveEvent, new LookTargets(pawn));
                Lv1_trait_gain = true;
            }
            //Lv.2 过目不忘
            if (0.15f <= Severity && Severity < 0.2f && !Lv2_trait_gain)
            {
                var trait_lv2 = new Trait(DefDatabase<TraitDef>.GetNamed("GreatMemory"));
                pawn.story.traits.GainTrait(trait_lv2);
                TaggedString resultText = string.Format("{0}", "BMN_GainTrait_text".Translate(pawn, trait_lv2.Label));
                Find.LetterStack.ReceiveLetter("BMN_GainTrait".Translate(), resultText, LetterDefOf.PositiveEvent, new LookTargets(pawn));
                Lv2_trait_gain = true;
            }
            //lv.3 敏捷
            if (0.25f <= Severity && Severity < 0.3f && !Lv3_trait_gain)
            {
                var trait_lv3 = new Trait(DefDatabase<TraitDef>.GetNamed("Nimble"));
                pawn.story.traits.GainTrait(trait_lv3);
                TaggedString resultText = string.Format("{0}", "BMN_GainTrait_text".Translate(pawn, trait_lv3.Label));
                Find.LetterStack.ReceiveLetter("BMN_GainTrait".Translate(), resultText, LetterDefOf.PositiveEvent, new LookTargets(pawn));
                Lv3_trait_gain = true;
            }
            //lv.4 受虐狂
            if (0.35f <= Severity && Severity < 0.4f && !Lv4_trait_gain)
            {
                var trait_lv4 = new Trait(DefDatabase<TraitDef>.GetNamed("Masochist"));
                pawn.story.traits.GainTrait(trait_lv4);
                TaggedString resultText = string.Format("{0}", "BMN_GainTrait_text".Translate(pawn, trait_lv4.Label));
                Find.LetterStack.ReceiveLetter("BMN_GainTrait".Translate(), resultText, LetterDefOf.PositiveEvent, new LookTargets(pawn));
                Lv4_trait_gain = true;
            }
            //lv.5 疾步如飞
            if (0.45f <= Severity && Severity < 0.5f && !Lv5_trait_gain)
            {
                var trait_lv5 = new Trait(DefDatabase<TraitDef>.GetNamed("SpeedOffset"), 2);
                pawn.story.traits.GainTrait(trait_lv5);
                TaggedString resultText = string.Format("{0}", "BMN_GainTrait_text".Translate(pawn, trait_lv5.Label));
                Find.LetterStack.ReceiveLetter("BMN_GainTrait".Translate(), resultText, LetterDefOf.PositiveEvent, new LookTargets(pawn));
                Lv5_trait_gain = true;
            }
            //lv.6 意志如钢
            if (0.55f <= Severity && Severity < 0.6f && !Lv6_trait_gain)
            {
                var trait_lv6 = new Trait(DefDatabase<TraitDef>.GetNamed("Nerves"), 2);
                pawn.story.traits.GainTrait(trait_lv6);
                TaggedString resultText = string.Format("{0}", "BMN_GainTrait_text".Translate(pawn, trait_lv6.Label));
                Find.LetterStack.ReceiveLetter("BMN_GainTrait".Translate(), resultText, LetterDefOf.PositiveEvent, new LookTargets(pawn));
                Lv6_trait_gain = true;
            }
            //lv.7 博闻强识
            if (0.65f <= Severity && Severity < 0.7f && !Lv7_trait_gain)
            {
                var trait_lv7 = new Trait(DefDatabase<TraitDef>.GetNamed("PerfectMemory"));
                pawn.story.traits.GainTrait(trait_lv7);
                TaggedString resultText = string.Format("{0}", "BMN_GainTrait_text".Translate(pawn, trait_lv7.Label));
                Find.LetterStack.ReceiveLetter("BMN_GainTrait".Translate(), resultText, LetterDefOf.PositiveEvent, new LookTargets(pawn));
                Lv7_trait_gain = true;
            }
            //lv.8 武术家(需要特性扩展)
            if(0.75f <= Severity && Severity < 0.8f && !Lv8_trait_gain && DefDatabase<TraitDef>.GetNamed("VTE_MartialArtist", false) != null)
            {
                var trait_lv8 = new Trait(DefDatabase<TraitDef>.GetNamed("VTE_MartialArtist"));
                pawn.story.traits.GainTrait(trait_lv8);
                TaggedString resultText = string.Format("{0}", "BMN_GainTrait_text".Translate(pawn, trait_lv8.Label));
                Find.LetterStack.ReceiveLetter("BMN_GainTrait".Translate(), resultText, LetterDefOf.PositiveEvent, new LookTargets(pawn));
                Lv8_trait_gain = true;
            }
        }
        ...
        public override void PostRemoved()
        {
            ...
            BMNUtility.RemoveFirstHediff(pawn, Hjx_BiomassNucleus.HediffDefOf.Hediff_Adaptiveform);
            pawn.story.traits.RemoveTrait(pawn.story.traits.GetTrait(DefDatabase<TraitDef>.GetNamed("BodyMastery")));
            ...
            if (ModLister.BiotechInstalled)
            {
                pawn.health.AddHediff(HediffDefOf.Hediff_ParenchymaCore);
            }
        }
    }
    ...
    public class Hediff_BiomassPartShield : Hediff_BiomassPartBase
    {
        ...
        public override void PostAdd(DamageInfo? dinfo)
        {
            ...
            {
                energy = baseNucleus.GetEnergy();
                if(baseNucleus.Severity >= 0.2 && baseNucleus.Severity < 0.4f)
                {
                    Severity = 0.1f;
                }else if(baseNucleus.Severity >= 0.4f && baseNucleus.Severity < 0.6f)
                {
                    Severity = 0.2f;
                }else if(baseNucleus.Severity >= 0.6f)
                {
                    Severity = 0.3f;
                }
            }
    ...
    public class Hediff_BiomassAdaptiveform : HediffWithComps
    {
        public bool hasNucleus
        {
            get
            {
                if (pawn.health.hediffSet.HasHediff(HediffDefOf.Hediff_BiomassNucleus))
                {
                    return true;
                }
                return false;
        ...
        public Apparel apparel_helmet = (Apparel)ThingMaker.MakeThing(ThingDefOf.PrototypeHelmet);

        public Apparel apparel_armor = (Apparel)ThingMaker.MakeThing(ThingDefOf.PrototypeArmor);
        ...
        {
            base.PostAdd(dinfo);
            pawn.health.AddHediff(RimWorld.HediffDefOf.RapidRegeneration);
            if (hasNucleus)
            {
                ...
                pawn.apparel.Wear(apparel_helmet);
                pawn.apparel.Lock(apparel_helmet);
                pawn.apparel.Wear(apparel_armor);
                pawn.apparel.Lock(apparel_armor);
                Severity = 0.1f;
                CompProperties_AbilityBiomassCost comp_cost = pawn.health.hediffSet.GetFirstHediffOfDef(HediffDefOf.Hediff_BiomassNucleus).def.abilities.FirstOrDefault((AbilityDef abil) => abil == AbilityDefOf.BMN_Evolution).comps.FirstOrDefault((AbilityCompProperties tmp) => tmp is CompProperties_AbilityBiomassCost) as CompProperties_AbilityBiomassCost;
                if (comp_cost != null)
                {
                    comp_cost.biomassCost = 60;
                }
                else
                {
                    Log.Message("Error");
                }
                var trait_bodymastert = new Trait(DefDatabase<TraitDef>.GetNamed("BodyMastery"));
                pawn.story.traits.GainTrait(trait_bodymastert);
            }
        ...
        {
            base.PostRemoved();
            pawn.apparel.Remove(apparel_helmet);
            pawn.apparel.Remove(apparel_armor);
            if(pawn.health.hediffSet.GetFirstHediffOfDef(RimWorld.HediffDefOf.RapidRegeneration) != null)
                pawn.health.RemoveHediff(pawn.health.hediffSet.GetFirstHediffOfDef(RimWorld.HediffDefOf.RapidRegeneration));
            if (hasNucleus)
            {
                Hediff_BiomassNucleus nucleus = pawn.health.hediffSet.hediffs.FirstOrDefault((Hediff h) => h is Hediff_BiomassNucleus) as Hediff_BiomassNucleus;
                nucleus.SetEnergy(nucleus.GetEnergy() + 20);
                pawn.story.traits.RemoveTrait(pawn.story.traits.GetTrait(DefDatabase<TraitDef>.GetNamed("BodyMastery")));
            }
            hasjudge = false;
        }
        ...
        public override void TickInterval(int delta)
        {
            ...
            if (pawn.IsHashIntervalTick(3600) && !hasjudge)
            {
                hasjudge = true;
                CompProperties_AbilityBiomassCost comp_cost = pawn.health.hediffSet.GetFirstHediffOfDef(HediffDefOf.Hediff_BiomassNucleus).def.abilities.FirstOrDefault((AbilityDef abil) => abil == AbilityDefOf.BMN_Evolution).comps.FirstOrDefault((AbilityCompProperties tmp) => tmp is CompProperties_AbilityBiomassCost) as CompProperties_AbilityBiomassCost;
                if (comp_cost != null)
                {
                    comp_cost.biomassCost = 0;
                }
            }
            if(pawn.IsHashIntervalTick(60) && hasNucleus)
            {
                Hediff_BiomassNucleus nucleus = pawn.health.hediffSet.hediffs.FirstOrDefault((Hediff h) => h is Hediff_BiomassNucleus) as Hediff_BiomassNucleus;
                HediffComp_Disappears hediffComp_Disappears = this.TryGetComp<HediffComp_Disappears>();
                if(hediffComp_Disappears != null && nucleus.GetEnergy() > 1f && hasjudge)
                {
                    hediffComp_Disappears.ticksToDisappear = 60 * (int)nucleus.GetEnergy();
                    nucleus.SetEnergy(nucleus.GetEnergy() - 1);
                    if(apparel_armor.HitPoints < apparel_armor.MaxHitPoints)
                    {
                        apparel_armor.HitPoints++;
                    }
                    if(apparel_helmet.HitPoints < apparel_helmet.MaxHitPoints)
                    {
                        apparel_helmet.HitPoints++;
                    }
                }
            }
        }
        ...
        {
            base.ExposeData();
            Scribe_References.Look(ref apparel_helmet, "BMN_apparel_helmet");
            Scribe_References.Look(ref apparel_armor, "BMN_apparel_armor");
            Scribe_Values.Look(ref hasjudge, "Has_judge", defaultValue: false);
        }
    }

    public class Hediff_CompProperties_AdaptiveOnDamage : HediffCompProperties
    {
        public List<DamageDef> damageDefs;

        public Hediff_CompProperties_AdaptiveOnDamage()
        {
            compClass = typeof(HediffComp_AdaptiveOnDamage);
        }
    }

    public class HediffComp_AdaptiveOnDamage : HediffComp
    {
        public Hediff_CompProperties_AdaptiveOnDamage Props => (Hediff_CompProperties_AdaptiveOnDamage)props;

        private List<float> Armor_rating_sharp = new List<float>{ 0.75f, 30, 0, 0.40f, 5, 0, 1};

        private List<float> Armor_rating_blunt = new List<float> { 0.75f, 30, 0, 0.40f, 5, 0, 1 };

        private List<float> Armor_rating_heat = new List<float> { 0.75f, 30, 0, 0.40f, 5, 0, 1 };

        public float num_sharp = 0;

        public float num_blunt = 0;

        public float num_heat = 0;

        private bool HasNucleus
        {
            get
            {
                if (parent.pawn.health.hediffSet.HasHediff(HediffDefOf.Hediff_BiomassNucleus))
                {
                    return true;
                }
                return false;
        ...
        private void Adaptive(DamageDef damageDef)
        {
            if(damageDef.armorCategory != null)
            {
                Hediff_BiomassAdaptiveform h = (Hediff_BiomassAdaptiveform)parent.pawn?.health?.hediffSet.GetFirstHediffOfDef(HediffDefOf.Hediff_Adaptiveform);
                if (HasNucleus)
                {
                    if (damageDef.armorCategory.defName == "Sharp")
                    {
                        Armor_rating_sharp[5] += 1;
                        if (Armor_rating_sharp[5] > 2 * Armor_rating_sharp[4] && Armor_rating_sharp[3] <= 200f)
                        {
                            Armor_rating_sharp[3] *= 2;
                            Armor_rating_sharp[4] *= 3;
                            Armor_rating_sharp[6] *= 3;
                        }
                        num_sharp = Calculate_sigmoid(Armor_rating_sharp[3], Armor_rating_sharp[5], Armor_rating_sharp[4], Armor_rating_sharp[6]);
                        StatUtility.SetStatValueInList(ref h.apparel_armor.def.statBases, DefDatabase<StatDef>.GetNamed("ArmorRating_Sharp"), num_sharp);
                        StatUtility.SetStatValueInList(ref h.apparel_helmet.def.statBases, DefDatabase<StatDef>.GetNamed("ArmorRating_Sharp"), num_sharp);
                    }
                    else if (damageDef.armorCategory.defName == "Blunt")
                    {
                        Armor_rating_blunt[5] += 1;
                        if(Armor_rating_blunt[5] > 2 * Armor_rating_blunt[4] && Armor_rating_blunt[3] <= 200f)
                        {
                            Armor_rating_blunt[3] *= 2;
                            Armor_rating_blunt[4] *= 3;
                            Armor_rating_blunt[6] *= 3;
                        }
                        num_blunt = Calculate_sigmoid(Armor_rating_blunt[3], Armor_rating_blunt[5], Armor_rating_blunt[4], Armor_rating_blunt[6]);
                        StatUtility.SetStatValueInList(ref h.apparel_armor.def.statBases, DefDatabase<StatDef>.GetNamed("ArmorRating_Blunt"), num_blunt);
                        StatUtility.SetStatValueInList(ref h.apparel_helmet.def.statBases, DefDatabase<StatDef>.GetNamed("ArmorRating_Blunt"), num_blunt);
                    }
                    else if (damageDef.armorCategory.defName == "Heat")
                    {
                        Armor_rating_heat[5] += 1;
                        if(Armor_rating_heat[5] > 2 * Armor_rating_heat[4] && Armor_rating_heat[3] <= 200f)
                        {
                            Armor_rating_heat[3] *= 2;
                            Armor_rating_heat[4] *= 3;
                            Armor_rating_heat[6] *= 3;
                        }
                        num_heat = Calculate_sigmoid(Armor_rating_heat[3], Armor_rating_heat[5], Armor_rating_heat[4], Armor_rating_heat[6]);
                        StatUtility.SetStatValueInList(ref h.apparel_armor.def.statBases, DefDatabase<StatDef>.GetNamed("ArmorRating_Heat"), num_heat);
                        StatUtility.SetStatValueInList(ref h.apparel_helmet.def.statBases, DefDatabase<StatDef>.GetNamed("ArmorRating_Heat"), num_heat);
                    }
                }
                int i = 0;
                if (HasNucleus)
                ...
                if (damageDef.armorCategory.defName == "Sharp")
                {
                    Armor_rating_sharp[2] += 1;
                    num_sharp = Calculate_sigmoid(Armor_rating_sharp[0], Armor_rating_sharp[2], Armor_rating_sharp[1]);
                    StatUtility.SetStatValueInList(ref h.def.stages[i].statOffsets, DefDatabase<StatDef>.GetNamed("ArmorRating_Sharp"), num_sharp);
                }
                else if (damageDef.armorCategory.defName == "Blunt")
                {
                    Armor_rating_blunt[2] += 1;
                    num_blunt = Calculate_sigmoid(Armor_rating_blunt[0], Armor_rating_blunt[2], Armor_rating_blunt[1]);
                    StatUtility.SetStatValueInList(ref h.def.stages[i].statOffsets, DefDatabase<StatDef>.GetNamed("ArmorRating_Blunt"), num_blunt);
                }
                else if (damageDef.armorCategory.defName == "Heat")
                {
                    Armor_rating_heat[2] += 1;
                    num_heat = Calculate_sigmoid(Armor_rating_heat[0], Armor_rating_heat[2], Armor_rating_heat[1]);
                    StatUtility.SetStatValueInList(ref h.def.stages[i].statOffsets, DefDatabase<StatDef>.GetNamed("ArmorRating_Heat"), num_heat);
                }
            }
        }

        public float Calculate_sigmoid(float factor, float x, float stand, float sqr = 1)
        {
            return (float)(factor / (1 + Math.Exp(-(x - stand)/sqr)));
        }

        public override void CompExposeData()
        {
            base.CompExposeData();
            Scribe_Collections.Look(ref Armor_rating_sharp, "BMNarmorRatingSharp");
            Scribe_Collections.Look(ref Armor_rating_blunt, "BMNarmorRatingBlunt");
            Scribe_Collections.Look(ref Armor_rating_heat, "BMNarmorRatingHeat");

            Scribe_Values.Look(ref num_sharp, "num_sharp");
            Scribe_Values.Look(ref num_blunt, "num_blunt");
            Scribe_Values.Look(ref num_heat, "num_heat");
        }
    }
    ...
    public static class HediffDefOf
    {
        ...
        [MayRequireBiotech]
        public static HediffDef BlackLightcoma;

        [MayRequireBiotech]
        public static HediffDef Hediff_ParenchymaCore;

        public static HediffDef Hediff_Adaptiveform;
    ...
    public class PawnRenderNodeWorker_Spastic : PawnRenderNodeWorker
    {
        public override Vector3 OffsetFor(PawnRenderNode node, PawnDrawParms parms, out Vector3 pivot)
        {
            ...
            {
                // 如果是伤痕节点且使用伤痕锚点
                if (node.hediff != null && node.Props.drawData.useBodyPartAnchor)
                {
            ...
            // 添加动画工作类的偏移
            Vector3 offset;
            if (node.AnimationWorker != null && node.TryGetAnimationOffset(parms, out offset) && !parms.flags.FlagSet(PawnRenderFlags.Portrait))
            {
                anchorOffset += offset;
            }
            // 添加痉挛特效偏移
        ...
        public override Quaternion RotationFor(PawnRenderNode node, PawnDrawParms parms)
        {
            ...
            // 添加动画工作类旋转
            float offset;
            if (node.AnimationWorker != null && node.TryGetAnimationRotation(parms, out offset) && !parms.flags.FlagSet(PawnRenderFlags.Portrait))
            {
                num += offset;
            }
    ...
    public static class ThingDefOf
    {
        ...
        public static ThingDef PrototypeHelmet;

        public static ThingDef PrototypeArmor;
    ...
    public class StartUp
    {
        static StartUp()
        {
            //Log.Message("BMN_Startup");
            var harmony = new Harmony("Hjx.BiomassNucleus");
            harmony.PatchAll();
        }
    }

    [HarmonyPatch]
    public class MannulPatch_Building_GeneExtractor
    {
        public static MethodInfo TargetMethod()
        {
            return AccessTools.Method("RimWorld.Building_GeneExtractor:Finish",
                new Type[0],
                null);
        }
        [HarmonyPrefix]
        public static void Prefix_Finish(Building_GeneExtractor __instance)
        {
            int randomInt = new System.Random().Next(1, 101);
            Pawn ContainedPawn = __instance.innerContainer.FirstOrDefault() as Pawn;
            if (ContainedPawn != null && ContainedPawn.genes.HasXenogene(GeneDefOf.BMN_Black_Light_Domaint) && randomInt > 66 && ContainedPawn.genes.Xenogenes.Where(ins => ins.def.displayCategory == GeneCategoryDefOf.Archite).Count() != 0)
            {
                Pawn containedPawn = ContainedPawn;
                List<GeneDef> genesToAdd = new List<GeneDef>();
                Genepack genepack = (Genepack)ThingMaker.MakeThing(RimWorld.ThingDefOf.Genepack);
                genesToAdd.Add(DefDatabase<GeneDef>.GetNamed("BMN_Black_Light_Domaint"));
                List<Gene> all_architegene = ContainedPawn.genes.Xenogenes.Where(ins => ins.def.displayCategory == GeneCategoryDefOf.Archite).ToList();
                genesToAdd.Add(all_architegene.RandomElement().def);
                if (genesToAdd.Any())
                {
                ...
                if (genesToAdd.Any())
                {
                    GenPlace.TryPlaceThing(genepack, intVec, __instance.Map, ThingPlaceMode.Near);
                }
                Messages.Message("BMN_GeneExtractionComplete".Translate(containedPawn.Named("PAWN")) + ": " + genesToAdd.Select((GeneDef x) => x.label).ToCommaList().CapitalizeFirst(), new LookTargets(containedPawn, genepack), MessageTypeDefOf.PositiveEvent);
            }
        }
    }
    ...
    {
        [MayRequireBiotech]
        public static GeneDef BMN_Black_Light_Domaint;

        [MayRequireBiotech]
        public static GeneDef BMN_Black_Light_Recessive;
    ...
    public class Gene_BL_recessive : Gene
    {
        private int Arc_num_max;

        private int Contain_Arc;

        public void Set_arcnum()
        {
            if(pawn?.ageTracker != null && pawn.ageTracker.AgeBiologicalYearsFloat <= 10f)
            {
                Arc_num_max = 2;
            }else if(pawn?.ageTracker != null && pawn.ageTracker.AgeBiologicalYearsFloat <= 30f)
            {
                Arc_num_max = 4;
            }
            else
            ...
            if(pawn.health?.hediffSet?.hediffs?.FirstOrDefault((Hediff h) => h is Hediff_BiomassNucleus) is Hediff_BiomassNucleus hediff_BiomassNucleus)
            {
                int num = (int)(hediff_BiomassNucleus.Severity / 0.1);
                if (num >= 4)
                    Arc_num_max += num;
            }
        }

        public void Get_containarc()
        {
            int num = 0;
            List<Gene> all_architeendogenes = pawn.genes.Endogenes.Where(ins => ins.def.displayCategory == GeneCategoryDefOf.Archite).ToList();
            if (all_architeendogenes.Count != 0)
                num = all_architeendogenes.Count;
            Contain_Arc = num;
        }
        ...
        {
            base.PostAdd();
            if (pawn.genes.HasXenogene(GeneDefOf.BMN_Black_Light_Recessive))
            {
                Gene gene = pawn.genes.Xenogenes.FirstOrDefault((Gene gene_delete) => gene_delete is Gene_BL_recessive) as Gene_BL_recessive;
                pawn.genes.RemoveGene(gene);
            }
            else
            {
                if (pawn.kindDef != DefDatabase<PawnKindDef>.GetNamed("Ghoul"))
                {
                    XenotypeDef evolver = DefDatabase<XenotypeDef>.GetNamed("Evolver");
                    pawn.genes.SetXenotype(evolver);
                    pawn.genes.AddGene(DefDatabase<GeneDef>.GetNamed("KindInstinct"), false);
                    if (!pawn.health.hediffSet.HasHediff(HediffDefOf.Hediff_BiomassNucleus))
                    {
                        pawn.health.AddHediff(HediffDefOf.Hediff_ParenchymaCore);
                    }
                ...
                {
                    string text = "Host_is_unqualified".Translate(pawn, pawn.kindDef.label);
                    Find.LetterStack.ReceiveLetter("Host_failed".Translate(), text, LetterDefOf.NegativeEvent, new LookTargets(pawn));
                    Gene gene = pawn.genes.Endogenes.FirstOrDefault((Gene gene_delete) => gene_delete is Gene_BL_recessive) as Gene_BL_recessive;
                    pawn.genes.RemoveGene(gene);
                }
            }
        }
        ...
        public override void TickInterval(int delta)
        {
            ...
            {
                Hediff firstHediffOfDef = pawn.health.hediffSet.GetFirstHediffOfDef(RimWorld.HediffDefOf.XenogermReplicating);
                if (firstHediffOfDef != null)
                {
                    int lest_ticks = firstHediffOfDef.TryGetComp<HediffComp_Disappears>().ticksToDisappear;
                    if (lest_ticks > 120000)
                    {
                        firstHediffOfDef.TryGetComp<HediffComp_Disappears>().ticksToDisappear = lest_ticks / 2;
                    }
                }
                firstHediffOfDef = pawn.health.hediffSet.GetFirstHediffOfDef(RimWorld.HediffDefOf.RegenerationComa);
                if (firstHediffOfDef != null)
                {
                    int lest_ticks = firstHediffOfDef.TryGetComp<HediffComp_Disappears>().ticksToDisappear;
                    if (lest_ticks > 120000)
            ...
            if (pawn.IsHashIntervalTick(300000))
            {
                Set_arcnum();
                Get_containarc();
                GeneDef gain_domaintgene = DefDatabase<GeneDef>.GetNamed("BMN_Black_Light_Domaint");
                if (pawn.genes.Xenotype != DefDatabase<XenotypeDef>.GetNamed("Evolver"))
                {
                    List<Gene> translate_genes = pawn.genes.Xenogenes.ListFullCopy();
                    pawn.genes.SetXenotype(DefDatabase<XenotypeDef>.GetNamed("Evolver"));
                    foreach (Gene gene in translate_genes)
                    {
                    ...
                    string text = "BMN_transform_xenotype".Translate(pawn);
                    Find.LetterStack.ReceiveLetter("BMN_transform".Translate(), text, LetterDefOf.NeutralEvent, new LookTargets(pawn));
                }
                if (!pawn.genes.HasXenogene(gain_domaintgene))
                    pawn.genes.AddGene(gain_domaintgene, true);
                if (!pawn.health.hediffSet.HasHediff(HediffDefOf.Hediff_ParenchymaCore) && !pawn.health.hediffSet.HasHediff(HediffDefOf.Hediff_BiomassNucleus))
                {
                    pawn.health.AddHediff(HediffDefOf.Hediff_ParenchymaCore);
                }
            }
            if (pawn.IsHashIntervalTick(1050000) && Contain_Arc < Arc_num_max)
            {
                Log.Message("Test");
                List<Gene> all_architexenoygene = pawn.genes.Xenogenes.Where(ins => ins.def.displayCategory == GeneCategoryDefOf.Archite && !pawn.genes.HasEndogene(ins.def)).ToList();
                if (all_architexenoygene.Count != 0)
                {
                    GeneDef gain_architegene = all_architexenoygene.RandomElement().def;
                    string text = "Abstract_Archite_Gene".Translate(pawn, gain_architegene.label);
                    Find.WindowStack.Add(new Dialog_MessageBox(text, "BMN_Confirm".Translate(Arc_num_max - Contain_Arc), delegate
                    {
                        pawn.genes.AddGene(gain_architegene, false);
                    }, "BMN_Cancel".Translate(), null));
                }
            }
            if (pawn.IsHashIntervalTick(1200000) && !pawn.health.hediffSet.HasHediff(RimWorld.HediffDefOf.DeathRefusal))
            {
                pawn.health.AddHediff(RimWorld.HediffDefOf.DeathRefusal);
            }
        }
        ...
        {
            base.ExposeData();
            Scribe_Values.Look(ref Arc_num_max, "arc_num_max", 2);
            Scribe_Values.Look(ref Contain_Arc, "contain_are", 0);
        }

    }

    public class Gene_BL_domaint : Gene
    {
        ...
        public void check()
        {
            ...
            if (!first)
            {
                ...
                {
                    first = true;
                    text = "first_success".Translate(pawn);
                    Find.LetterStack.ReceiveLetter("check_domaint".Translate(), text, LetterDefOf.PositiveEvent, new LookTargets(pawn));
                }
                else
                {
                    text = "first_failed".Translate(pawn);
                    pawn.health.AddHediff(HediffDefOf.BlackLightcoma);
                    Find.LetterStack.ReceiveLetter("check_domaint".Translate(), text, LetterDefOf.NegativeEvent, new LookTargets(pawn));
                }
            }
            else if (!second)
            {
                ...
                {
                    second = true;
                    text = "second_success".Translate(pawn);
                    Find.LetterStack.ReceiveLetter("check_domaint".Translate(), text, LetterDefOf.PositiveEvent, new LookTargets(pawn));
                }
                else
                {
                    text = "second_failed".Translate(pawn);
                    pawn.health.AddHediff(HediffDefOf.BlackLightcoma);
                    Find.LetterStack.ReceiveLetter("check_domaint".Translate(), text, LetterDefOf.NegativeEvent, new LookTargets(pawn));
                }
            }
            else if (!third)
            {
                ...
                {
                    third = true;
                    text = "third_success".Translate(pawn);
                    Find.LetterStack.ReceiveLetter("check_domaint".Translate(), text, LetterDefOf.PositiveEvent, new LookTargets(pawn));
                    pawn.genes.AddGene(GeneDefOf.BMN_Black_Light_Recessive, false);
                }
                else
                {
                    text = "third_failed".Translate(pawn);
                    pawn.health.AddHediff(HediffDefOf.BlackLightcoma);
                    Find.LetterStack.ReceiveLetter("check_domaint".Translate(), text, LetterDefOf.NegativeEvent, new LookTargets(pawn));
                }
            }
        ...
        {
            base.TickInterval(delta);
            if (pawn.IsHashIntervalTick(360000) && !pawn.genes.HasEndogene(GeneDefOf.BMN_Black_Light_Recessive))
            {
                check();
            }
        }
        ...
        {
            base.ExposeData();
            Scribe_Values.Look(ref first, "BMN_first", defaultValue: false);
            Scribe_Values.Look(ref second, "BMN_second", defaultValue: false);
            Scribe_Values.Look(ref third, "BMN_third", defaultValue: false);
        }
    }
}
